...
public interface DeudaNominaRepository extends JpaRepository<DeudaNomina, Long> {
	...
	@Query(value = "select d.id, d.cuit, to_char(d.periodo, 'MM/YYYY') periodo, a.entidad, d.aporte, a.descripcion aporteDescripcion, "
			...
			+ "	   d.acta_id, d.acta_pago, (select distinct max(numero) from molineros_acta ma where ma.id = d.acta_id and a.entidad = fentidad_empleadores_get(ma.entidad) ) as acta_nro, "
			+ "	   d.convenio_id, d.convenio_pago, (select distinct max(convenio_numero) from molineros_convenio mc where mc.id = d.convenio_id and a.entidad = fentidad_empleadores_get(mc.entidad) ) as convenio_nro "
			+ " from   deuda_nomina d "
